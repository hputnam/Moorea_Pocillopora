...
chl_path <- "data_pocillopora_timeseries/3_March_2022/3_chlorophyll/"                                        # Path to chlorophyll data directory
...
homog.vol <- read_csv("data_pocillopora_timeseries/3_March_2022/3_homogenate_vols.csv") %>%
  select(colony_id, homog_vol_ml)
chl <- full_join(df, homog.vol)

# Load surface area
sa <- read_csv("data_pocillopora_timeseries/3_March_2022/3_surface.area.calc.csv")
...
chl %>%
  ...
  write_csv(path = "data_pocillopora_timeseries/3_March_2022/3_chlorophyll.csv")
```

# Plot results by species and site
```{r, eval = TRUE}
# Join with sample metadata
meta <- read_csv("data_pocillopora_timeseries/3_March_2022/3_corals_sampled.csv")
chl <- right_join(chl, meta, by = "colony_id")

# Plot chlorophyll a
chl %>%
  ggplot(aes(x = site.x, y = chla.ug.cm2, color = timepoint)) +
  #facet_wrap(~species) +
...
# Plot chlorophyll c2
chl %>%
  ggplot(aes(x = site.x, y = chlc2.ug.cm2, color = timepoint)) +
 # facet_wrap(~species) +
