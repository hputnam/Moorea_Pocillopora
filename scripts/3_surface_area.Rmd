...
#load wax data
wax.data <- read.csv("gametogenesis_timeseries/March_2022/data/2_surface_area/2_surface_area_data.csv", header=TRUE)
...
smpls%>%
  mutate(timepoint="March2022")%>%
  select(colony_id, surface.area.cm2, timepoint)%>%
    write_csv("gametogenesis_timeseries/March_2022/output/surface.area.calc.csv")
```
