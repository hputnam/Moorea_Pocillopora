...
#load wax data
wax.data <- read.csv("data/surface_area_data.csv", header=TRUE)

wax.data$delta.mass.g <- wax.data$weight2.g-wax.data$weight1.g
stnds <- subset(wax.data, sample.type=="Standard")
...
smpls <- subset(wax.data, sample.type=="Coral")
...
smpls %>%
  count(full_sample_id) %>% arrange(n)
...
smpls%>%
  select(full_sample_id, surface.area.cm2, date)%>%
    write_csv("output/surface.area.calc.csv")
```
