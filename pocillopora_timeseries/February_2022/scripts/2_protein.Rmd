---
title: "February 2022 host and holobiont protein analysis"
...
prot_path = "February_2022/data/3_protein/3_host/"                                              # Path to prot data directory
...
```{r}
# Surface area data
sa <- read.csv("February_2022/output/3_surface.area.calc.csv")

# Tissue homogenate volume data
homog_vols <- read_csv("February_2022/data/3_homogenate_vols/3_homogenate_vols.csv") %>% select(1:3)

# Coral sample metadata
metadata <- read_csv("February_2022/3_corals_sampled.csv") %>% select(1:5)
...
prot_host %>%
  ...
  write_csv(., path = "February_2022/output/3_host_protein.csv")
```
...
```{r}
# List protein data files 
prot_path = "February_2022/data/3_protein/3_holobiont/"                                              # Path to prot data directory
all_prot_files <- list.files(path = prot_path, pattern = "*.csv")          # List all files in directory
prot_platemaps <- list.files(path = prot_path, pattern = "platemap")       # List platemap files
prot_data_files <- setdiff(all_prot_files, prot_platemaps)                 # List data files
...
prot_holobiont %>%
  ...
  select(colony_id, prot_ug, prot_ug.cm2) %>%
  mutate(timepoint="FEB")%>%
  write_csv(., path = "February_2022/output/3_holobiont_protein.csv")
```
